...
import { Printer, Download, TrendingUp, ShoppingBag, Banknote, QrCode, ChevronLeft, ChevronRight } from 'lucide-react'
      ...
      <div className="max-w-screen-xl mx-auto space-y-4">
        ...
        {tab === 'transaksi' && summary && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 no-print">
            {[
              { label: 'Total Pendapatan', value: formatRupiah(summary.totalRevenue), icon: TrendingUp },
              { label: 'Jumlah Transaksi', value: String(summary.totalTransactions), icon: ShoppingBag },
              { label: 'Transaksi Cash', value: String(summary.cashCount), icon: Banknote },
              { label: 'Transaksi QRIS', value: String(summary.qrisCount), icon: QrCode },
            ].map(card => (
...
function usePagination<T>(items: T[], defaultSize = 10) {
...
function PaginationFooter({
  page, setPage, pageSize, setPageSize, totalPages, totalItems,
}: {
  page: number
  ...
  return (
    <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100 bg-gray-50 flex-wrap gap-2 no-print">
      ...
      <div className="flex items-center gap-1">
        <button onClick={() => setPage(1)} disabled={page === 1}
          className="px-2 py-1.5 rounded-lg text-xs font-medium text-gray-500 hover:bg-gray-200 disabled:opacity-30 disabled:cursor-not-allowed transition-colors">«</button>
...
function TransaksiTable({ transactions }: { transactions: Transaction[] }) {
  ...
  const { page, setPage, pageSize, setPageSize, totalPages, paginated } = usePagination(transactions, 10)
  ...
  return (
    <>
      ...
      <PaginationFooter
        ...
        totalPages={totalPages} totalItems={transactions.length}
      />
    </>
  )
}
...
function ProdukTable({ products, tab }: { products: Product[], tab: Tab }) {
  const { page, setPage, pageSize, setPageSize, totalPages, paginated } = usePagination(products, 10)

  if (!products.length) return <div className="text-center py-16 text-gray-400">Tidak ada data</div>

  const colCount = 4 + (tab !== 'hampir_habis' ? 1 : 0) + (tab === 'stok' || tab === 'kadaluarsa' ? 1 : 0)
  ...
  return (
    <>
      ...
      <PaginationFooter
        page={page} setPage={setPage}
        pageSize={pageSize} setPageSize={setPageSize}
        totalPages={totalPages} totalItems={products.length}
      />
    </>
  )
}
